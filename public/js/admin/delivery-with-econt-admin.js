jQuery( document ).ready( function (dwe) {    
  ...
  function onPreview () {
    ...
    let payment_method = t.data("payment_method");
    let order_status = t.data("order_status");
    ...
    if(payment_method == 'econt_payment' && order_status == 'pending' && !waybill_id) {
        if (!confirm("По тази поръчка няма резервирана сума. Искате ли да я изпратите с обикновен наложен платеж?")) {
            return;
        } else {
            update_waybill_status_processing(order_id)
        }
    }
    ...
    r.then( (response) => {
      // setTimeout( () => {
        ...
        if ( response && response.shipmentNumber ) {
          ...
          }) && dwe('.wc-backbone-modal-main').css({paddingBottom:0}) : ( t.addClass( "disabled" )
          ,
            dwe.ajax({
              ...
              success:function( e ){                               
                ...
                e.success&&(
                  ...
                  dwe( this ).WCBackboneModal({
                    template:"dwe-modal",
                    variable:e.data
                  }) && dwe('.wc-backbone-modal-main').css({paddingBottom:0})
                )
              }
            })
          ), !1
  ...
  window.addEventListener( 'message', function( message ) {
      ...
      if (message.data.event === 'cancel') {
          ...
          u.then(() => {
              if (u) {
                  ...
                  dwe("#action-waybill-" + global_order_id).data().waybillId = global_waybill_id;
                  jQuery('.modal-close').click();
    ...
    function update_waybill(message) {
        ...
        return dwe.ajax({
            ...
            dataType: 'json',
            success: function () {
                global_waybill_id = JSON.parse(message).shipmentStatus.shipmentNumber;
    ...
    function update_waybill_status_processing(order_id) {
        return dwe.ajax({
            url:delivery_with_econt_admin_object.ajax_url,
            data:{
                "order_id": order_id,
                "action": "delivery_with_econt_update_status_processing",
                "security": delivery_with_econt_admin_object.security
            },
            type:"POST",
            dataType: 'json',
